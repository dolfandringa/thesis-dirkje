...
## Calculate diversity and number of nursery fish species

```{r diversity_sign_species}

sig_species <-df.species_all[c(2,40,52,54,58,88,97,103),]
...
```{r glm_div_loop}
  res <- glm(formula = sig_nurs_diversity ~ log(distance_seagrass) * log(size_seagrass) * mpa * depth * north, data = df.sample_data)
...
``` {r likelihood_diversity}
  model1 <- glm(formula = sig_nurs_diversity ~ log(distance_seagrass) * log(size_seagrass) * mpa * depth * north, data = df.sample_data)
  model2 <- glm(formula = sig_nurs_diversity ~ log(distance_seagrass) * log(size_seagrass) * mpa * north, data = df.sample_data)
...
```{r glm_sp_loop}
  res <- glm(formula = sig_nurs_species ~ log(distance_seagrass) * log(size_seagrass) * mpa * depth * north, data= df.sample_data)
...
``` {r likelihood_species}
  model1 <- glm(formula = sig_nurs_species ~ log(distance_seagrass) * log(size_seagrass) * mpa * depth * north, data = df.sample_data)
  model2 <- glm(formula = sig_nurs_species ~ log(distance_seagrass) * log(size_seagrass) * mpa * north, data = df.sample_data)
  lrtest(model1, model2)
  #>0.05 = model 2 predicts data significantly better than model 1 {eliminate variabele with highest p-value}
  #<0.05 = model 2 predicts data significantly less than model 1 {end result}
```
