...
```{r readfile}
df.raw <- read_ods(path='C:\\Users\\Dirkje\\Desktop\\R\\Data_Surveys_dirkje.ods',sheet=2,col_types = c(text,numeric,numeric,numeric,numeric,numeric,numeric,numeric,numeric,date))
df.species <- as.data.frame(read_ods(path='C:\\Users\\Dirkje\\Desktop\\R\\indicator_species2.ods',sheet=1,col_types = c(text,text)))
df.species_dirkje_only <- as.data.frame(read_ods(path='C:\\Users\\Dirkje\\Desktop\\R\\indicator_species2.ods',sheet=2,col_types = c(text,text)))
df.species_nursery <- as.data.frame(read_ods(path='C:\\Users\\Dirkje\\Desktop\\R\\indicator_species2.ods',sheet=3,col_types = c(text,text)))
...
df.sites$distance_mangroves <- sample(1:1000, 9)
df.sites$size_mangroves <- sample(1:400, 9)
...
```{r save}
save(df.raw,df.processed,df.sample_data,df.species_all,df.sites,list=c("df.raw","df.processed","df.sample_data","df.species_all","df.sites"), file="C:\\Users\\Dirkje\\Desktop\\R\\data.RDa")
```
